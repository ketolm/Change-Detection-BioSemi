...
def main(fCue,fStim,fInterval,fTest,fPause,recLength,recWidth,cubeLength,cubeWidth,tPerBlock,stimType,targChange,trialType,cue,numTrials):
    ...
    # COLORS
    recColors  = [[['red','blue'],['red','blue']],[['blue','blue','blue','blue'],['blue','blue','blue','blue']],[['red','red','blue','blue'],['red','red','blue','blue']]]
    blues      = ['blue', 'cyan', 'darkblue']
    reds       = ['red', 'lightsalmon', 'darkred']
    ...
    def recDraw(frames, nStim, stimulus):
                frames = frames
                fixation.draw()
                fixation2.draw()
                ...
                for frameN in range(frames):
                    ...
                    for kbe in events:
                        if cue[t] == 0:
                            if kbe.char == 'r':
                                trialResp[t] = 1
                                responseTime[t] = kbe.time - startT
                            elif kbe.char == 'f':
                                trialResp[t] = 0
                                responseTime[t] = kbe.time - startT
                        else:
    ...
    def cubeDraw(frames, nStim, stimulus):
                frames = frames
                fixation.draw()
                fixation2.draw()
    ...
    for t in range(numTrials):
        ...
        for n in range(2):
            ...
            if rectangles:
                ...
                for j in range(nStim):
                    ...
                    for i in range(3):
                        # numCube x 3 sides
                        cubeStim[n][j][i] = ShapeStim(win,vertices = cubeV[n][j][i], fillColor = cubeColors[trialType[t]][n][j][i], size = .5)
        ...
        # presents stim
        if rectangles:
            recDraw(fStim, nStim, recStim)
        else:
            cubeDraw(fStim, nStim, cubeStim)
        ...
        if rectangles:
            ...
            recDraw(fTest, nStim, recStim)
        else:
            if targChange[t]:
                stimChange = random.randint(0,nStim-1)
                while not 'blue' in cubeColors[trialType[t]][cue[t]][stimChange]:
                    stimChange = random.randint(0,nStim-1)
